...
onMounted(async () => {
  ...
  try {
    ...
    console.log("Data source from store:", dataSource);
    if (!dataSource) {
      throw new Error("No data source selected");
    }
    console.log("Fetching tables for data source:", dataSource);
    ...
    if (result.success && Array.isArray(result.tables)) {
      ...
      console.log("Tables set:", tables.value);
    ...
    console.error("Error in TableSelector:", err);
    error.value = err.message || "An error occurred while fetching tables";
  } finally {
    loading.value = false;
    console.log("TableSelector loading complete. Tables:", tables.value);
  }
});

function submitSelection() {
  console.log("Submitting table selection:", selectedTables.value);
